...
bun run og         # redraw the per-route Open Graph cards (opens a local page; --headless drives Chrome instead)
...
bun run scan-sheets # build docs/scan-sheets.pdf, the printable sheets for the real-phone scan matrix (--headless; CI runs this)
bun run logo-fixtures # check the SVG-logo fixtures in a real browser (--headless; CI runs this)

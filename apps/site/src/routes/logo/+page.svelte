...
<section class="mx-auto max-w-7xl px-4 sm:px-6">
	<div class="prose">
		...
		<p>A logo can only occupy a whole odd number of modules, so on a small code the sizes it can be are a short staircase. The Size control steps through that staircase, one position per size the code can actually produce, and the readout names each as a share of the code's width; every step changes the logo, and nothing on the slider is a dead spot. In Advanced the readout also says how much of the code the logo hides, which is the number the warnings are set against.</p>
		<p>The crop box under the logo is the blank space in the middle of the code. Drag it to choose what goes there and drag its corner to change its size and shape: the space is cut to the box's shape, so a wordmark cropped to its mark gets a square space, and a logo exported with padding all round can be tightened until it fills what it is given. An SVG logo cropped this way is still a drawing in the SVG download.</p>
		...
		<ul>
			<li>Use a square or round version of your logo if you have one, such as the icon or monogram, or crop the box down to that part of a wordmark. A wide logo is fitted to the width of the space, so it comes out short and hard to read at arm's length.</li>
			...
			<li>Convert text in an SVG to outlines before uploading. Live text falls back to whatever font the reader has, which is rarely yours.</li>
